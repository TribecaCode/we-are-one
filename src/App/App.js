...
const defaultSearch = {
  ...
  keyword: null,
  sort:
    window.location.pathname === "/schedule" ? "screening_date_time" : "title"
};
...
function App(props) {
  ...
  const onSearch = e => {
    let new_state = { ...state, keyword: e.target.value };
    const new_selected_films = filterFilms(state.all_events, new_state);
    new_state = { ...new_state, selected_events: new_selected_films };
    setState(new_state);
  };
...
function filterFilms(films, state) {
  ...
  if (state.selected_kinds.length) {
    remaining_films = remaining_films.filter(film =>
      state.selected_kinds.includes(film.kind)
    );
  }

  // filter by date
  if (state.selectedDate) {
    remaining_films = remaining_films.filter(
      film => film.date && film.date === state.selectedDate
    );
  }
  ...
  if (state.selected_festival) {
    remaining_films = remaining_films.filter(
      film => state.selected_festival === film.festival_name
    );
  }

  if (state.keyword) {
    let re = RegExp(state.keyword, "gi");
    remaining_films = remaining_films.filter(film =>
      film.display_title.match(re)
    );
  }
