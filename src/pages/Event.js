  ...
  return (
    <React.Fragment>
      ...
      <div className="hidden-xs col-sm-12">
        <div className="Film">
          <Link
            className={`Film_img-container Film_img-container--${item.color}`}
            style={{ backgroundImage: `url(${img_src})` }}
            to={`/events/${item.slug}`}
            item={item}
          />
          <div className="Film_content">
            <div>
              <div className="pull-right">
                <div
                  className="Film_curated"
                  onClick={onFestivalToggle(item.festival_name)}
                >
                  Curated by{" "}
                  <span className="f_title">{item.festival_name}</span>
                </div>
                <div className="Film_runtime pull-right">
                  {item.run_time} min
                </div>
              </div>
              ...
              <h4 className="heading-4">
                <Link className="Film_title" to={`/events/${item.slug}`}>
                  {item.display_title}
                </Link>
            ...
            <div className="time_and_more">
              ...
              <Link className="Film_more" to={`/events/${item.slug}`}>
                View Details
              </Link>
            </div>
          </div>
